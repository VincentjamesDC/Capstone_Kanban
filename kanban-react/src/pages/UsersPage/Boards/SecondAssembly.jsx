...
const SecondAssembly = () => {
    ...
    const DoForm = {
        ...
        assembly_two_start: defaultTimeValue+" "+user.name,
        assembly_two_finish: "",
        action : 'Do'
    }
    ...
    const DoneForm = {
        assembly_two: "Done",
        assembly_two_start: "",
        assembly_two_finish: defaultTimeValue+" "+user.name,
        action : 'Done'
    }
    ...
    const handleDoProcess = async (e, order_id) => {
      ...
      const handleUndone = async (order_id) => {
        const response = await axios.put("api/enrod/assembly_two/" + order_id, UndoneForm);
        if(response.data.status === 200){
            await getOrders();
      ...
      const handleUndo = async (order_id) => {
        const response = await axios.put("api/enrod/assembly_two/" + order_id, UndoForm);
        if(response.data.status === 200){
            await getOrders();
            setUndoRes(response.status);
            closeUndoResult();
                                        ...
                                        <div  key={order.id} className="m-auto h-full w-full max-w-md bg-white shadow-md p-2 border-t-4 border-amber-600 rounded">
                                               <header className="px-2 py-1 border-b flex justify-between"> 
                                                ...
                                                <div className="flex flex-wrap justify-between p-2 w-full gap-2">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <h4 className="text-xs">Started By: {order.assembly_two_start.split(' ').slice(1).join(' ')}</h4>
                                                        <h1 className="text-md">{order.assembly_two_start?.split(' ')[0]}</h1>
                                                    </div>
                            ...
                            return(
                                    <div  key={order.id} className="m-auto h-full w-full max-w-md bg-white shadow-md p-2 border-t-4 border-green-600 rounded">
                                            ...
                                            <div className="flex flex-wrap justify-between p-2 w-full gap-2">
                                                ...
                                                <div className="flex flex-col mr-2">
                                                    <h4 className="text-xs">Finished By: {order.assembly_two_finish.split(' ').slice(1).join(' ')}</h4>
