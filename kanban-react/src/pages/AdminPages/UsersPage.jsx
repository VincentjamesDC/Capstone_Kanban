...
const UsersPage = () => {
  ...
  const {users, user, getUsers, deleteUser, handleUserRestore, res_result, createAdmin, closeResResultFast, del_result, closeDelResultFast, onChange, onChange2, formValues2, setFormvalues, formValues, errors, showModal, showModal2, setModal, setModal2, createUser, result, closeResultFast, loading, handleSoftDelete } = useAuthContext();
    ...
    return (
    <div>
        <div className='md:px-32 flex gap-2'>
            ...
            {/* <button onClick={() => setModal2(true)} className='px-4 py-2 bg-blue-600 text-sm font-medium hover:bg-blue-500 duration-100 transition-all ease-out text-white rounded-md mt-10'>New Admin</button> */}
        </div>
        <div className='md:px-32 mt-4'>
        <div className="inline-flex w-fit overflow-hidden  divide-x rounded-lg bg-gray-800 rtl:flex-row-reverse divide-gray-700">
                        ...
                        <button onClick={() => {setActive("active")}} className={active == "active" ? "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm text-white bg-blue-600" : "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm hover:bg-gray-700 text-gray-200"}>
                            Active
                        </button>

                        <button onClick={() => {setActive("inactive")}} className={active == "inactive" ? "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm text-white bg-blue-600" : "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm hover:bg-gray-700 text-gray-200"}>
                            Blocked
                        </button>
                    </div>
        </div>
        <div className="md:px-32 py-4 w-full">
            <div className="shadow overflow-hidden rounded border-b border-gray-200 max-h-[50vh] overflow-y-scroll">
                ...
                <thead className="bg-gray-800 text-white">
                    ...
                    {
                        active !== 'active' && <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Blocked At</th>
                    }
                ...
                <tbody className="text-gray-700">
                    {
                    active == 'active' ? 
                    users?.filter((user)=> user.role=="User" && user.deleted_at === null).map(user => {
                        return(
                            <tr key={user.id}>
                                <td className=" text-left py-3 px-4">{user.name}</td>
                                <td className=" text-left py-3 px-4">{user.email}</td>
                                <td className=" text-left py-3 px-4">{user.department}</td>
                                <td className="text-left py-3 px-4">{user.role}</td>
                                {/* <td className="w-1/3 text-left py-3 px-4 text-red-500">template</td> */}
                                <td className=" text-left py-3 px-4">
                                    <button onClick={() => handleDelete(user.id, user.name)} className='px-4 py-2 bg-red-600 text-sm font-medium hover:bg-red-500 duration-100 transition-all ease-out text-white rounded-md'>Block</button>
                                </td>
                            </tr>
                        )
                    })
                    ...
                    users?.filter((user)=> user.role=="User" && user.deleted_at !== null).map(user => {
                        return(
                            <tr key={user.id}>
                                ...
                                <td className=" text-left py-3 px-4">{user.department}</td>
                                <td className="text-left py-3 px-4">{user.role}</td>
                                <td className="text-left py-3 px-4">{user.deleted_at}</td>

                                {/* <td className="w-1/3 text-left py-3 px-4 text-red-500">template</td> */}

                                <td className="text-left py-3 px-4">
                                    <button onClick={() => handleRestore(user.id, user.name)} className='px-4 py-2 bg-green-600 text-sm font-medium hover:bg-green-500 duration-100 transition-all ease-out text-white rounded-md'>Restore</button>
                                </td>
        ...
        </div>
        {
            ...
            <div className='pb-20'>
                ...
                <div className="inline-flex w-fit overflow-hidden  divide-x rounded-lg bg-gray-800 rtl:flex-row-reverse divide-gray-700">
                                ...
                                <button onClick={() => {setActive2("active")}} className={active2 == "active" ? "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm text-white bg-blue-600" : "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm hover:bg-gray-700 text-gray-200"}>
                                    Active
                                </button>

                                <button onClick={() => {setActive2("inactive")}} className={active2 == "inactive" ? "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm text-white bg-blue-600" : "px-5 py-2 text-xs font-medium transition-colors duration-200 sm:text-sm hover:bg-gray-700 text-gray-200"}>
                                    Blocked
                                </button>
                            </div>
                </div>
                <div className="md:px-32 py-4 w-full">
                    <div className="shadow overflow-hidden rounded border-b border-gray-200">
                        ...
                        <thead className="bg-gray-800 text-white">
                            ...
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Role</th>
                            {
                                active2 !== 'active' && <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Deleted At</th>
                        ...
                        <tbody className="text-gray-700">
                            {
                            active2 == 'active' ? 
                            users?.filter((user)=> user.role=="Admin" && user.deleted_at === null).map(user => {
                                return(
                                    <tr key={user.id}>
                                        <td className=" text-left py-3 px-4">{user.name}</td>
                                        <td className=" text-left py-3 px-4">{user.email}</td>
        ...
        {
            showModal2 && 
            <FadeInOut show={showModal2} duration={200}>
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-lg md:h-auto">
                ...
                <div className="relative bg-white rounded-lg shadow-lg">
                    <button onClick={() => setModal2(false)}  type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center " data-modal-hide="authentication-modal">
                    ...
                    <div className="px-6 py-6 lg:px-8">
                        ...
                        <form className="space-y-6" onSubmit={createAdmin}>
                            <div>
                                ...
                                <input type="text" name="name" id="name" value={formValues2["name"]} onChange={onChange2} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required/>
                                {errors.name && (<div className='flex'><span className="text-red-400 text-sm m-2 p-2">{errors.name[0]}</span></div>)}

                            </div>
                            <div>
                                <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                                <input type="email" name="email" id="email" value={formValues2["email"]} onChange={onChange2} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="name@company.com" required/>
                            ...
                            <div>
                                ...
                                <input type="password" name="password" id="password" placeholder="••••••••" value={formValues2["password"]} onChange={onChange2} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required/>
                            ...
                            <div>
                                ...
                                <input type="password" name="password_confirmation" id="password_confirmation" placeholder="••••••••" value={formValues2["password_confirmation"]} onChange={onChange2} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required/>
        ...
        {
            restoreModal && 
            <FadeInOut show={restoreModal} duration={200}>
            <div id="authentication-modal" tabIndex="-1" aria-hidden="true" className="fixed top-0 bg-zinc-800/60 left-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-lg md:h-auto">

                <div className="relative bg-white rounded-lg shadow-lg">
                    <button onClick={() => setRestoreModal(false)}  type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center " data-modal-hide="authentication-modal">
                        <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                        <span className="sr-only">Close modal</span>
                    </button>
                    <div className="px-6 py-6 lg:px-8">
                        <h3 className="mb-4 text-xl font-medium text-gray-900 ">Restore User: {delName}?</h3>
                        <div className='flex gap-2'>
                        <button type="button" onClick={() => submitRestore()} className="flex items-center justify-center w-full text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                            {
                                loading ? <div role="status">
                                <svg aria-hidden="true" className="w-5 h-5 mr-2 text-gray-200 animate-spin dark:text-gray-300 fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                                ...
                                <span className="sr-only">Loading...</span>
                            </div> : <span className="inline-block " id='sign-in'>Restore</span>
                            }
                            
                            
                        </button>
                        <button onClick={() => setRestoreModal(false)} className="w-full text-gray-800 bg-gray-300 hover:bg-gray-400 transition-all duration-100 ease-out border focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                          Cancel
                        </button>
                        </div>
                    </div>
        ...
        {
            res_result && 
            <FadeInOut show={res_result} duration={150}>
                <div className='w-1/6 absolute bottom-2 right-6'>
                    <div id="alert-border-3" className="flex p-4 mb-4 border-t-4  text-white bg-gray-800 border-green-800 rounded-md" role="alert">
                        <svg className="flex-shrink-0 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path></svg>
                        <div className="ml-3 text-sm font-medium">
                        ...
                        <button onClick={closeResResultFast} type="button" className="ml-auto -mx-1.5 -my-1.5   rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 inline-flex h-8 w-8 bg-gray-800 text-green-400 hover:bg-gray-700"  data-dismiss-target="#alert-border-3" aria-label="Close">
                        <span className="sr-only">Dismiss</span>
                        <svg aria-hidden="true" className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                        </button>
                    </div>
