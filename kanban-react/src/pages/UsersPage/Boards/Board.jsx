...
const Board = () => {
    ...
    const DoForm = {
        cutting: "In-Progress",
        cutting_start: defaultTimeValue+" "+user.name,
        cutting_finish: "",
        action : 'Do'
    }
    ...
    const DoneForm = {
        cutting: "Done",
        cutting_start: "",
        cutting_finish: defaultTimeValue+" "+user.name,
        action : 'Done'
    }
    ...
    const [undo_res, setUndoRes ] = useState(null);
        ...
        e.preventDefault();
        const response = await axios.put("api/enrod/cutting/" + order_id, DoForm);
      ...
      const handleDone = async (e, order_id) => {
        e.preventDefault();
        const response = await axios.put("api/enrod/cutting/" + order_id, DoneForm);
        if(response.data.status === 200){
            await getOrders();
      ...
      const handleUndone = async (order_id) => {
        const response = await axios.put("api/enrod/cutting/" + order_id, UndoneForm);
        if(response.data.status === 200){
            await getOrders();
      ...
      const handleUndo = async (order_id) => {
        const response = await axios.put("api/enrod/cutting/" + order_id, UndoForm);
        if(response.data.status === 200){
            await getOrders();
            setUndoRes(response.status);
            closeUndoResult();
                        ...
                        product_orders?.filter(product_order => product_order.cutting === "In-Progress").length > 0 ?
                                ...
                                return(
                                        <div  key={order.id} className="m-auto h-full w-full max-w-md bg-white shadow-md p-2 border-t-4 border-amber-600 rounded">
                                                ...
                                                <div className="flex flex-wrap justify-between p-2 w-full gap-2">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <h4 className="text-xs">Started By: {order.cutting_start.split(' ').slice(1).join(' ')}</h4>
                                                        <h1 className="text-md">{order.cutting_start?.split(' ')[0]}</h1>
                                                    </div>
                            ...
                            <div className="m-auto h-52 invisible w-full max-w-md bg-white shadow-md p-2 border-t-4 border-amber-600 rounded">
                                ...
                                <div className="flex flex-wrap justify-between p-2 w-full gap-2">
                                    ...
                                    <div className="flex flex-col">
                                        <h4 className="text-xs">Date Started</h4>
                                        <h1 className="text-md"></h1>
                                    </div>
                            ...
                            return(
                                    <div  key={order.id} className="m-auto h-full w-full max-w-md bg-white shadow-md p-2 border-t-4 border-green-600 rounded">
                                            ...
                                            <div className="flex flex-wrap justify-between p-2 w-full gap-2">
                                                ...
                                                <div className="flex flex-col">
                                                    <h4 className="text-xs">Finished By: {order.cutting_finish.split(' ').slice(1).join(' ')}</h4>
                                                    <h1 className="text-md">{order.cutting_finish?.split(' ')[0]}</h1>
                                                </div>
                                            </div>
