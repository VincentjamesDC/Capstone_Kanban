...
const SecondAssembly = () => {
    ...
    const DoneForm = {
        ...
        date_assembled_one: "",
        assembly_two: "Ok",
        date_assembled_two: defaultTimeValue,
        quality_control: "",
        date_checked: "",
        finishing_one: "",
        date_finished_one: "",
    ...
    const handleDoProcess = async (e, order_id) => {
        e.preventDefault();
        const response = await axios.put("api/enrod/assembly_two/" + order_id, DoForm);
        if(response.data.status === 200){
            getOrders();
            navigate("/dashboard/assembly_two");
        }
      };

      const handleDone = async (e, order_id) => {
        e.preventDefault();
        const response = await axios.put("api/enrod/assembly_two/" + order_id, DoneForm);
        if(response.data.status === 200){
            getOrders();
            navigate("/dashboard/assembly_two");
        }
    ...
    <div className='py-5 md:px-12'>
            ...
            <div className="h-full overflow-y-auto">
                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <div>
                        ...
                        {product_orders?.filter(product_order => product_order.assembly_two === null  && product_order.assembly_one === "Ok").length}
                        </p>
                    </div>
                    </div>
                    <div className="flex items-center p-4 rounded-lg shadow-xs bg-gray-800">
                    ...
                    <div>
                        ...
                        <p className="text-lg font-semibold text-gray-200">
                        {product_orders?.filter(product_order => product_order.assembly_two === "In-Progress" && product_order.assembly_one === "Ok").length}
                        </p>
                    </div>
                    </div>
                    <div className="flex items-center p-4 rounded-lg shadow-xs bg-gray-800">
                    ...
                    <div>
                        ...
                        <p className="text-lg font-semibold text-gray-200">
                        {product_orders?.filter(product_order => product_order.date_assembled_two !== null && product_order.date_assembled_one !==null).length}
                        </p>
                    </div>
                    </div>
                </div>
        ...
        <div className='px-4 gap-6 flex w-full flex-wrap lg:flex-nowrap'>
                ...
                <div className='flex flex-col gap-4 max-h-[60vh] overflow-y-scroll'>
                    {
                        ...
                        product_orders?.filter(product_order => (product_order.assembly_two === null && product_order.assembly_one === "Ok")).map(order => {
                            return(
            ...
            <div className="py-8 w-full lg:w-1/3">
                ...
                <div className='flex flex-col gap-4 max-h-[60vh] overflow-y-scroll'>
                    {
                        product_orders?.filter(product_order => product_order.assembly_two === "In-Progress"  && product_order.assembly_one === "Ok").length > 0 ?
                        
                            product_orders?.filter(product_order => (product_order.assembly_two === "In-Progress" && product_order.assembly_one === "Ok")).map(order => {
                                return(
            ...
            <div className="py-8 w-full lg:w-1/3">
                ...
                <div className='flex flex-col gap-4 max-h-[60vh] overflow-y-scroll'>
                    {
                        product_orders?.filter(product_order => (product_order.assembly_two === "Ok" && product_order.assembly_one === "Ok")).map(order => {
                            return(
